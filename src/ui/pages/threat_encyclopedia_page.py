    ...
    def _inject_to_dashboard(self, packet):
        ...
        if self.dashboard and hasattr(self.dashboard, "packet_data"):
            ...
            if len(self.dashboard.packet_data["packets"]) > 1000:
                self.dashboard.packet_data["packets"] = self.dashboard.packet_data["packets"][
                    -1000:
                ]

            # Update packet count
            self.dashboard.packet_data["packet_count"] = (
                self.dashboard.packet_data.get("packet_count", 0) + 1
            )
...
class SimulationController(QWidget):
    ...
    def __init__(self, engine, dashboard=None, parent=None):
        super().__init__(parent)
        self.engine = engine
        self.dashboard = dashboard
        ...
        # Quick start guide shown flag
        self.quick_start_shown = False
        ...
        self.status_label.setStyleSheet(
            "color: #5BA4B3; font-size: 12px; padding: 5px; background-color: #0F1218; border-radius: 4px;"
        )
        ...
        self.lbl_packets.setStyleSheet("color: #76D7EA; font-weight: bold; font-size: 12px;")
        self.lbl_rate = QLabel("Rate: 0 pkt/s")
        ...
        stats_layout.addSpacing(20)
        stats_layout.addWidget(self.lbl_threat)
        ...
        self.packet_log.setPlaceholderText(
        ...
        self.summary_log.setPlaceholderText(
            "Human-readable threat explanations will appear here..."
        )
        ...
        self.btn_pause.setObjectName("StopBtn")
    ...
    def _start_attack(self, threat_name):
        """Start an attack simulation with confirmation dialog."""
        if self.engine and not self.engine.is_running():
            ...
            msg.setText(f"Starting: {threat_name}")
            msg.setInformativeText(f"""
<strong>What to expect:</strong>
• {packet_count} packets will be injected
• Duration: ~{duration:.1f} seconds
• Interval: {interval}ms between packets

<strong>Description:</strong>
{template.get('desc', 'No description available')}
...
No real harm will occur to your system.
            ...
            if msg.exec() == QMessageBox.StandardButton.Ok:
                ...
                if not self.quick_start_shown:
                    self._show_quick_start_guide()
                    self.quick_start_shown = True
    ...
    def _toggle_pause(self):
        ...
        if self.engine:
            if self.is_paused:
                ...
                self.status_label.setText(f"Status: RUNNING: {self.engine.active_simulation}")
            else:
                self.engine.timer.stop()
                self.is_paused = True
                self.btn_pause.setText("▶Resume")
                self.status_label.setText(f"Status: PAUSED: {self.engine.active_simulation}")
...
<strong>Tip:</strong> These are safe simulations - no real harm occurs!
    ...
    def _on_sim_started(self, threat_name, desc):
        ...
        self.status_label.setStyleSheet(
            "color: #9B59B6; font-weight: bold; padding: 10px; background-color: #0F1218; border-radius: 5px;"
        )
    ...
    def _on_sim_finished(self, threat_name):
        ...
        self.status_label.setStyleSheet(
            "color: #5BA4B3; font-weight: bold; padding: 10px; background-color: #0F1218; border-radius: 5px;"
        )
        self.btn_stop.setEnabled(False)
        self.btn_pause.setEnabled(False)
        self.btn_pause.setText("⏸Pause")
    ...
    def _on_packet_injected(self, packet):
        """Handle packet injection - display in log with plain English summary."""
    ...
    def _get_plain_english_summary(self, packet):
        """Generate plain English summary of the packet."""
        threat = packet.get("threat_type", "UNKNOWN")
        src = packet.get("src_ip", "unknown")
        dst = packet.get("dst_ip", "unknown")
        port = packet.get("port", "-")
...
class ThreatCard(QFrame):
    ...
    def __init__(
        ...
        risk_color = (
            ...
            else ("#FFA500" if "HIGH" in risk.upper() else "#76D7EA")
        )
        ...
        how_title.setStyleSheet(
            f"color: {self.CYAN_DIMMER}; text-transform: uppercase; margin-top: 15px;"
        )
...
class ThreatEncyclopediaPage:
    ...
    def _create_header(self):
        ...
        self.search_input.returnPressed.connect(
            lambda: self._filter_threats(self.search_input.text())
        )
