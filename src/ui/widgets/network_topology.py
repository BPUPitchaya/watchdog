...
class NetworkTopologyWidget(QWidget):
    ...
    def _draw_connections(self, painter, center, orbit_radius, gateway_radius):
        ...
        for i, _device in enumerate(self.devices):
            ...
            for j in range(packet_count):
                ...
                dist_to_gateway = math.sqrt(
                    (packet_x - center.x()) ** 2 + (packet_y - center.y()) ** 2
                )
    ...
    def _draw_gateway(self, painter, center, radius):
        ...
        icon_rect = QRect(
            int(center.x() - icon_size // 2), int(center.y() - icon_size // 2), icon_size, icon_size
        )
        painter.drawRect(icon_rect)
        painter.drawLine(
            icon_rect.center().x(), icon_rect.top(), icon_rect.center().x() - 5, icon_rect.top() - 8
        )
        painter.drawLine(
            icon_rect.center().x(), icon_rect.top(), icon_rect.center().x() + 5, icon_rect.top() - 8
        )
    ...
    def _draw_device_node(self, painter, center, radius, device, index):
        ...
        painter.setFont(QFont(THEME["font_mono"].strip("'"), 10))
        ip_rect = QRect(
            int(center.x() - radius - 20), int(center.y() + radius + 2), radius * 2 + 40, 16
        )
        painter.setPen(QPen(QColor(THEME["text_secondary"])))
        ...
        if hostname and hostname != "Unknown":
            host_rect = QRect(
                int(center.x() - radius - 20), int(center.y() + radius + 18), radius * 2 + 40, 14
            )
    ...
    def _draw_legend(self, painter):
        ...
        for i, (_type_id, label, color) in enumerate(types):
