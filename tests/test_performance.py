...
class PerformanceTester:
    ...
    def test_high_throughput(self, target_pps=1000, duration=5):
        ...
        # Performance criteria
        success_rate = (packet_count - failed_packets) / packet_count * 100 if packet_count > 0 else 0
    ...
    def generate_report(self):
        ...
        if 'high_throughput' in self.results:
            ...
            print(f"  Success rate: {((self.results['high_throughput']['packets_processed'] - self.results['high_throughput']['failed_packets']) / self.results['high_throughput']['packets_processed'] * 100):.1f}%")
