...
class LiveTrafficWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        chart_rect = QRectF(
            chart_left, chart_top, chart_right - chart_left, chart_bottom - chart_top
        )
        ...
        status_color = (
            QColor(76, 175, 80) if self.network_status == "Connected" else QColor(255, 107, 107)
        )
        ...
        # Max at top
        painter.drawText(
            int(chart_rect.left() - 30), int(chart_rect.top() + 5), str(self.y_axis_max)
        )
        painter.drawText(int(chart_rect.left() + 10), int(chart_rect.top() + 5), "Pkts/s")
        # Half at middle
        painter.drawText(
            int(chart_rect.left() - 30), int(chart_rect.center().y() + 3), str(self.y_axis_max // 2)
        )
        # 0 at bottom
        ...
        for i in range(num_points):
            x = (
                chart_rect.left() + i * (width / (num_points - 1))
                if num_points > 1
