...
class LocalSettings:
    ...
    def __init__(self, settings_file: str | None = None) -> None:
        if settings_file is None:
            ...
            # Check if running as a frozen app or if we don't have write permissions in current directory
            if hasattr(sys, "frozen") or os.getcwd() == "/":
                home = os.path.expanduser("~")
                app_dir = os.path.join(home, ".watchdog")
                try:
                    os.makedirs(app_dir, exist_ok=True)
                    self.settings_file = os.path.join(app_dir, "local_settings.json")
                except Exception:
                    self.settings_file = "local_settings.json"
            else:
                self.settings_file = "local_settings.json"
        else:
