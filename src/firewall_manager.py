...
class FirewallManager:
    ...
    def __init__(self, table_name: str = "watchdog_blocked") -> None:
    ...
    def _run_pfctl(self, args: list) -> str | None:
        """Run pfctl with sudo."""
        cmd = ["sudo", "pfctl"] + args
        try:
            ...
            print(
                "Error: pfctl command not found. pf firewall may not be available on this system."
            )
            return None
        except Exception as e:
    ...
    def block_ip(self, ip: str, timeout: int | None = None) -> bool:
        """Block an IP address."""
        try:
