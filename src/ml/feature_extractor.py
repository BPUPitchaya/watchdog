...
class FeatureExtractor:
    ...
    def __init__(self, window_size: int = 100, time_window: float = 2.0) -> None:
        """
    ...
    def extract_packet_features(self, packet_data: dict) -> dict:
        ...
        try:
            ...
            if not src_ip or not dst_ip:
                logger.warning("Missing src_ip or dst_ip in packet data")
            ...
            if time_diff > 0:
                ...
                features["same_srv_rate"] = (
                    conn["same_srv_count"] / conn["count"] if conn["count"] > 0 else 0
                )
                features["diff_srv_rate"] = (
                    conn["diff_srv_count"] / conn["count"] if conn["count"] > 0 else 0
                )
            ...
            dst_host_count = sum(
                1
                for p in self.global_packets
                if p.get("dst_ip") == dst_ip
                and current_time - p.get("timestamp", 0) < self.time_window
            )
            ...
            log_exception(logger, "feature extraction", e)
            logger.error(f"Failed to extract features from packet: {e}")
            return self._get_default_features()
    ...
    def get_selected_features(self, features_dict: dict) -> tuple:
        ...
        try:
            ...
            log_exception(logger, "getting selected features", e)
            logger.error(f"Failed to get selected features: {e}")
            # Return default features
            return [0] * 20, selected
